...
try:
    ...
    class Microphone(AudioSource):
        ...
        used as the audio source. Otherwise, ``device_index`` should be the index
        of the device to use for audio input.
        """
        ...
        def __enter__(self):
            ...
            self.stream = self.audio.open(
                ...
                format=self.format, rate=self.RATE, channels=self.CHANNELS,
...
class WavFile(AudioSource):
    ...
    If ``filename_or_fileobject`` is a string, then it is interpreted as a path to
    a WAV audio file on the filesystem. Otherwise, ``filename_or_fileobject``
...
class Recognizer(AudioSource):
    def __init__(self, language="pt-BR", key="AIzaSyBOti4mM-6x9WDnZIjIeyEU21OpBXqWBgw"):
        """
        Creates a new ``Recognizer`` instance, which represents a collection of
        speech recognition functionality.
        The language is determined by ``language``, a standard language code like
        `"en-US"` or `"en-GB"`, and defaults to US English. A list of supported
        language codes can be found `here <http://stackoverflow.com/questions/14257598/>`__.
        Basically, language codes can be just the language (``en``), or a language
        with a dialect (``en-US``).
        ...
        # seconds of quiet time before a phrase is considered complete
        self.pause_threshold = 0.8

         # amount of quiet time to keep on both sides of the recording
    ...
    def samples_to_flac(self, source, frame_data):
        ...
        # directory of the current module file
        ...
        if flac_converter is None:
            ...
            if system == "Windows" and platform.machine() in processors:
                ...
                raise OSError(
                    ...
                    the FLAC command line application using `brew install flac` or
                    your operating system's equivalent"""
                )
    ...
    def record(self, source, duration=None):
        ...
        (an ``AudioSource`` instance) into an ``AudioData`` instance,
        which it returns.
    ...
    def adjust_for_ambient_noise(self, source, duration = 1):
        ...
        Should be used on periods of audio without speech - will stop early if
        any speech is detected.
        ...
        This value should be at least 0.5 in order to get a representative sample
    ...
    def listen(self, source, timeout = None):
        """
        Records a single phrase from ``source`` (an ``AudioSource`` instance) into
        an ``AudioData`` instance, which it returns.
        ...
        seconds of silence or there is no more audio input. The ending silence is
        not included.
    ...
    def recognize(self, audio_data, show_all = False):
        ...
        otherwise it returns a ``dict`` of all possible transcriptions and their
        confidence levels.

        Note: confidence is set to 0 if it isn't given by Google

        Also raises a ``LookupError`` exception if the speech is unintelligible, a
        ``KeyError`` if the key isn't valid or the quota for the key has been
        maxed out, and ``IndexError`` if there is no internet connection.
        """
        ...
        self.request = Request(
            ...
            headers={"Content-Type": "audio/x-flac; rate=%s" % audio_data.rate}
        )
    ...
    def listen_in_background(self, source, callback):
        ...
        (an ``AudioSource`` instance) into an ``AudioData`` instance and call
        ``callback`` with that ``AudioData`` instance as soon as each phrase are
        detected.

        Returns the thread (a ``threading.Thread`` instance) immediately,
        while the background thread continues to run in parallel.

        Phrase recognition uses the exact same mechanism as
        ``recognizer_instance.listen(source)``.

        The ``callback`` parameter is a function that should accept two
        parameters - the ``recognizer_instance``, and an ``AudioData`` instance
        representing the captured audio. Note that this function will be called
